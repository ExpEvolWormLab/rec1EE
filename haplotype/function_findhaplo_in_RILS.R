...
# KeepMostFrequentHaplo = function(haplotype, snps){
#   
#   
#   hapfreq = do.call(rbind, lapply(1:nrow(snps), function(i){
#     if(i %% 1000 == 0){print(i)}
#     pos = snps$pos[i]
...
KeepMostFrequentHaplo = function(haplotype, foundernames){
  
  
  breakpoints = sort(unique(c(haplotype$whichsnp1, haplotype$whichsnp2)))
  
  #breakpoints = c(1,10,20,30)
  
  hapfreq = do.call(rbind, lapply(1:(length(breakpoints)-1), function(i){
    if(i %% 1000 == 0){print(paste0(round(100*i/length(breakpoints), digits=1),"%"))}
    
    brki = breakpoints[i]
    brkj = breakpoints[i+1]
    nsnp = brkj-brki
    if(i == length(breakpoints)-1) nsnp = nsnp+1
    
    hap = haplotype[haplotype[,"whichsnp1"] <= brki & haplotype[,"whichsnp2"] > brki,]
    freq = apply(hap[,foundernames], 2, sum)/nrow(hap)
    freq= matrix(freq,nrow=1)
    
    if(nsnp>1) freq = freq[rep(1, nsnp),]
    
    colnames(freq)=foundernames
    freq
  }))
  ...
  haplotype = do.call(rbind, lapply(1:nrow(haplotype), function(i){
    #print(i)
    if(i %% 1000 == 0){print(paste0(round(100*i/nrow(haplotype), digits=1),"%"))}
    x = haplotype[i,]
    wfounder = x[,foundernames]>0
    if(sum(wfounder)>1){
      whichsnp1 = x[,"whichsnp1"]
      whichsnp2 = x[,"whichsnp2"]
      
      freq = hapfreq[whichsnp1:whichsnp2,wfounder]
      if(whichsnp2!=whichsnp1){freq = apply(freq, 2, mean)}
      ismax = which.max(freq)
      wfounder = which(wfounder)[ismax]
      x[,foundernames] = 0
      x[,foundernames[wfounder]] = 1
    }
...
putBreakAtMidDistance=function(HAPLO,colnames.distance=c("cM1", "cM2"), round=F){
  
  
  HAPLO = HAPLO[order(HAPLO[,colnames.distance[1]],HAPLO[,colnames.distance[2]]),]
  
  for(i in 1:(nrow(HAPLO)-1)){
    j = i + 1
    
    if(HAPLO[i,"whichsnp2"]>HAPLO[j,"whichsnp1"]){
      di = HAPLO[i,colnames.distance[2]]
      dj = HAPLO[j,colnames.distance[1]]
      
      newd = (di+dj)/2
      
      if(round){newd=round(newd)}
      
      HAPLO[i,colnames.distance[2]] = newd
      HAPLO[j,colnames.distance[1]] = newd+1
    }
  }
...
divideOverlapHapBlocksByFounderID = function(haplotype, foundercolnames=c("FA.g1", "FA.g2", "FM.g1", "FM.g2"), info=NULL){
  ...
  haplotype=haplotype[haplotype$nfounder>0,]
...
haploPlotFromat = function(Rilshaplotypes, widthRILs = 0.85, foundercolnames=c("FA.g1", "FA.g2", "FM.g1", "FM.g2"),
                           founderscolors = c("#AA497F","#644329", "#428D4D", "#D47C3E"),na.color='black', info, orderedRils = NULL){
  ...
  haplop = do.call(rbind, lapply(split(Rilshaplotypes, Rilshaplotypes$rilname), function(haplotype){
    ...
    haplotype=divideOverlapHapBlocksByFounderID(haplotype, foundercolnames=foundercolnames,info=NULL)
  ...
  if(is.null(orderedRils = NULL)) orderedRils = sort(unique(haplop$rilname))
  
  rilY = as.numeric(factor(haplop$rilname, levels = orderedRils))-1
  
  haplop$y1 = rilY + (haplop$rely1)*widthRILs
  haplop$y2 = rilY + (haplop$rely2)*widthRILs
  
  if(!is.null(founderscolors) & !is.null(na.color)){
    haplop$haplocolor=founderscolors[match(haplop$founder, foundercolnames)]
    haplop$haplocolor[is.na(haplop$haplocolor)]=na.color
  }
